import {
	// @ts-ignore -- __unstableEditorStyles exists at runtime but is not typed
	__unstableEditorStyles as EditorStyles,
	// @ts-ignore -- __unstableIframe exists at runtime but is not typed
...
export interface GlobalStyles {
	settings?: {
		typography?: {
			fontFamilies?: {
				theme?: Array< { name: string; fontFamily: string } >;
			};
		};
		color?: {
			palette?: {
...
export default function StylesPreview( {
	...
	// Typography
	const fontFamily =
		( getStyleValue( mergedStyles, 'typography.fontFamily' ) as string ) ?? 'serif';
	...
	const h1FontFamily = getStyleValue( mergedStyles, 'typography.fontFamily', 'h1' ) as string;
	const headingFontFamily = getStyleValue(
		mergedStyles,
	...
	const colorVarPalette = currentColorVariation?.settings?.color?.palette?.theme;

	const activeBackgroundColor = () => {
		return resolveColor(
			backgroundColor,
			currentColorVariation?.styles?.color?.background,
			globalPalette,
			colorVarPalette,
			'white'
		);
	...
	const activeTextColor = () => {
		return resolveColor(
			textColor,
			currentColorVariation?.styles?.color?.text,
			globalPalette,
			colorVarPalette,
			'black'
		);
