...
export default function useAbilitiesRegistration( deps: AbilitiesDeps ): void {
	// Updated every render so abilities always read fresh values.
	...
	useEffect( () => {
		...
		( async () => {
			...
			try {
				await registerAbility( showComponentAbility( () => depsRef.current.showComponent ) );
			} catch {
				// Ability may already be registered.
			}
		} )();
	}, [] );
}
